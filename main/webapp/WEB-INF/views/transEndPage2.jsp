...
<link href="css/easy-responsive-tabs.css" rel="stylesheet"
	type="text/css">
<link href="css/jquerysctipttoprecipt.css" rel="stylesheet"
	type="text/css">
...
<body>
	...
	<div class="container payment-section-wishlist">
		<div
			class="col-lg-12 col-md-12 col-sm-12 img-values-col txt-cnt-wait-n">
			<%
			...
			<img
				...
				class="" /> <a class="navbar-brand" href="<%=clientLogoLink%>"><span>
					<%=collegeBean.getCollegeName()%>,<br><%=collegeBean.getStateBean().getStateName()%>
			</span></a>
			<%
				}else{
			%>
			<img src="img\No-logo.jpg"> <a href="<%=clientLogoLink%>"><span>
					<%=collegeBean.getCollegeName()%>,<br><%=collegeBean.getStateBean().getStateName()%>
			</span></a>
		...
		<div class="col-lg-12 col-md-12 col-sm-12">
			<div class="col-lg-12 col-md-12 col-sm-12 price-summary">
				<div class="col-lg-12 col-md-12 col-sm-12 txt-cnt-wait-n greenlab">
					Save Transaction receipt for successful Transactions for future use
					by clicking the 'View receipt' button. <br>
					<c:if
						test="${status.contentEquals('pending') || status.contentEquals('failure')}">
				...
				<div class="col-lg-12 col-md-12 col-sm-12 price-title no-pad">
					...
					<span class="payee-head">Transaction Date : <span
						class="flr">
						<fmt:parseDate value="${beanTransData.transDate}" pattern="yyyy-MM-dd'T'HH:mm" var="parsedDateTime" type="both" />
						<c:out value="${parsedDateTime}" />
						<%-- <fmt:formatDate pattern="dd.MM.yyyy HH:mm" value="${ parsedDateTime }" /> --%>
						<%-- <c:date name="${beanTransData.transDate}" /> --%></span></span> 
				...
				<div class="cntrs- labeling-pad" style="float: right;">
					...
					<c:if test="${status.contentEquals('success') }">
						<span id="btnClicks"><input type="submit"
							id="btnSubmit-first"
							onclick='viewFormlst("<c:out value='${beanTransData.id}'/>","<c:out value="${beanTransData.transId}" />")'
							value="View receipt" class="btn btn-grn btm-margs"></span>
					</c:if>
				</div>
			</div>
			<div align="center">
				...
				<button type="button" onclick="goToStart()"
					class="wizard-goto btn btn-primary">Back to Home</button>
		...
		function goToStart(){
			...
			if(redirectedFromBankFlag=='Bank'){
				 if(bid=='4'){
					...
					window.location='Start.jsp?bid=<%=sesBid%>&cid=<%=sesCid%>&redirectedFrom=Bank&currentSessionReturn=Y';
				 }
			}
			else{				
				window.location='Start.jsp?bid=<%=sesBid%>&cid=<%=sesCid%>&currentSessionReturn=Y';
			}
