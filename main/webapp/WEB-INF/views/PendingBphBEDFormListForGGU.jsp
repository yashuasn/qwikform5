...
<s:token/>
					...
					<div class="box col-md-12">
						<div class="box-inner">
							<div class="box-header well" data-original-title="">
								<h2>
									<i class="glyphicon glyphicon-star-empty"></i>Pending Applicants List For Approval
								</h2>
							...
							<div class="box-content">
								...
								<table id="transTable" class='display nowrap'>
									<thead>
										<tr>
											<c:forEach items="${pendingRecordsHeaders}" var="fieldList" varStatus="ind">
												<c:choose>
													<c:when test="${fn:contains(fieldList, 'unique_Key')}">
														<th>Approve Applicant</th>
									...
									<c:forEach items="${pendingRecordVal}" var="Outer" varStatus="ind">
										<c:if test="${fn:contains(Outer,'NA')}">
										<tr>
	...
	function clickForVerification(uniqueKey){
		...
		var flag="yes";
		var pending="pending";
		var course="BPharma_Bed";
		//alert(formId+" : "+cid+" : "+cIdForApproval+" : "+uniqueKey);
		if (confirm('Do you want to approve this applicant? If yes then press OK. otherwise press cancel.')) {
			alert('Applicant is approved successfully.');
			window.location = "ApplicantReportsForApprovalProcess?formId="+formId+"&cid="+cid+"&uniqueKey="+uniqueKey+"&approveFlag="+flag+"&flag="+pending+"&course="+course;
			
		} else {
			  alert('The Applicant is not approved.');
		}
	...
	function getApplicantDetailsBasedONFeeType() {
			...
			if(cid===cIdForApproval){
				if(formId!=null || formId!=""){
					...
					window.location = "ApplicantReportsForApprovalProcess?formId=" + formId+"&cid="+cid+"&flag="+pending+"&course="+course;
				}else{
