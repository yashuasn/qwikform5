...
<s:token/>
					...
					<div class="box col-md-12">
						<div class="box-inner">
							<div class="box-header well" data-original-title="">
								<h2>
									<i class="glyphicon glyphicon-star-empty"></i>Approved Applicants List
								</h2>
								<input type="hidden" id="isFormId" name="isFormId" value='<c:out value="${currentFormId }"/>' />
							</div>
			...
			model.put("pendingRecordVal", pendingRecordVal);
			model.put("approvedRecordsHeaders", approvedRecordsHeaders);
			model.put("approvedRecordVal", approvedRecordVal);
								%> --%>
								<table id="transTable" class='display nowrap'>
									<thead>
										<tr>
											<c:forEach items="${approvedRecordsHeaders}" var="fieldList" varStatus="ind">
												<c:choose>
													<c:when test="${fn:contains(fieldList, 'unique_Key')}">
														<th>Disapprove Applicant</th>
									...
									<c:forEach items="${approvedRecordVal}" var="Outer" varStatus="ind">
										<c:if test="${fn:contains(Outer,'approve')}">
										<tr>
											<c:forEach items="${Outer}" var="bean">
												<c:choose>
													<c:when test="${fn:contains(bean, cCodeForApproval)}">
														<td>&zwnj;
															<button type="button" onclick="clickForVerification('<c:out value="${bean}" />')"
																class="btn btn-success btn-sm">Disapprove</button>
														</td>
														<%-- <td>&zwnj;
															<c:out value="${bean}"/>
														</td> --%>
	...
	function clickForVerification(uniqueKey){
		var formId=document.getElementById("isFormId").value;
		var cid=<%=collegeID%>;
		var cIdForApproval=<%=cIdForApproval%>;
		var flag="yes";
		var approve="approve";
		//alert(formId+" : "+cid+" : "+cIdForApproval+" : "+uniqueKey);
		if (confirm('Do you want to disapprove this applicant? If yes then press OK. otherwise press cancel.')) {
			alert('Applicant is disapproved successfully.');
			window.location = "ApplicantReportsForApprovalProcess?formId="+formId+"&cid="+cid+"&uniqueKey="+uniqueKey+"&disApproveFlag="+flag+"&flag="+approve;
			
		} else {
			  alert('The Applicant is not disapproved.');
		}
	...
	function getApplicantDetailsBasedONFeeType() {
			...
			if(cid===cIdForApproval){
				if(formId!=null || formId!=""){
					...
					window.location = "ApplicantReportsForApprovalProcess?formId=" + formId+"&cid="+cid+"&flag="+approve;
				}else{
