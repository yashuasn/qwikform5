...
<body onload="ExecuteFun();">
	...
	<%
			...
			/* ArrayList<SampleFormView> formViewData = new ArrayList(); */
				...
				try {
					...
					/* log.info("FormSummaryNew 3May.jsp::formInstanceId is:"+formInstanceIdSummary);
					log.info("FormSummaryNew 3May.jsp::flag for hasInstruction is:"+flag1);
					log.info("FormSummaryNew 3May.jsp::flag for movetoPG is:"+flag);
					log.info("FormSummaryNew 7May.jsp::collegeBean with collegecode  is:"+collegeBean.getCollegeCode()); */
					...
					log.info("FormSummaryNew 3May.jsp::formBean.FormId is:"+formBean.getFormId());
					log.info("FormSummaryNew 3May.jsp::currentFormId is:"+currentFormId);
	...
	<%
				...
				//log.info("FormSummaryNew 3May.jsp::filePath for filePath is:"+filePath);
			%>
					...
					<table class="table-td" style="width: 100%; align-items: left">
						...
						<tr>
							<td colspan="2"><u><b>Note:</b></u> Your in the process of
								...
								above mentioned fields you may cancel or go back to edit the
								required fields</td>
						</tr>
						<%} %>
								...
								<form name="goBackForms" id="backGo" class="form-horizontal"
									...
									<input type="hidden" name="dob" class="form-control"
										...
										class="form-control" value="<%=formid%>"> <span><button
											...
											onClick="backToForms()">Go Back</button></span> <span><button
											type="button" class="wizard-prev btn btn-primary rht-gap"
											onClick='window.location="getPayerFormsById1?formid=<%=formBean.getFormId()%>&bid=<%=sesBid %>&cid=<%=sesCid %>&PayeeProfile=<%=PayeeProfile%>&flag=0"'>Previous</button></span>
	...
	function downloadFile(url){
		...
		';
			...
			alert("useable URL is " + 'getPayerFormsById?formid=' + formid
					+ '&bid=' + bid + '&cid=' + cid + '&PayeeProfile='
					+ PayerProfile);
			window.location = 'getPayerFormsById?formid=' + formid + '&bid='
					+ bid + '&cid=' + cid + '&PayeeProfile=' + PayerProfile;
		}
	</script>
</body>
