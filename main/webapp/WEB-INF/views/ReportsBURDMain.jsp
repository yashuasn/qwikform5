...
<s:token/>
									...
									<td>
										<div class="control-group">
												...
												<div class="controls" style="height: 40px;width:">
													<select name="fd.paymentCat" class="form-control" id="colId" data-rel="chosen"
														...
														<option selected="selected" disabled="disabled" value="">Select
															An Option</option>
														<c:forEach items="${collegeNameList}" var="bean1">
															<option value='${bean1}'>
															${bean1}
															</option>
														</c:forEach>
				...
				<div class="row" id="ResultBox">
					<div class="box col-md-12">
						<div class="box-inner">
							<div class="box-header well" data-original-title="">
								<h2>
									<i class="glyphicon glyphicon-star-empty"></i>Seat Filling status as per College Name 
								</h2>
							</div>
							<div class="box-content">
								<!-- put your content here -->
								...
								<table id="transTable" class='display nowrap' width="100%">
									<tr>
										...
										<c:forEach items="${subjectListForMainClient}" var="OuterList" varStatus="ind">
											<c:set var="subId">
												<c:out value="${OuterList.id}" />
											</c:set>
											<c:set var="subName">
												<c:out value="${OuterList.subject_name}" />
											</c:set>
											
											<c:forEach items="${seatList }" var="seatListNew">
												<c:if test='${subId==seatListNew.subject_id }'>
												<tr>
													<td align="center"> <c:out value="${subName}"/> </td>	
													<td align="center"> <c:out value="${seatListNew.subject_max_seat}"/> </td>	
													<td align="center"> <c:out value="${seatListNew.subject_filled_seat}"/> </td>	
												</tr>
												</c:if>
											</c:forEach>
	...
	function getApplicantDetailsBasedONFeeType() {
			...
			var colName=document.getElementById("colId").value;
			
			window.location = "ApplicantReportsForAllClientsForBUClient?colName="+colName;
	    	//window.location = "ApplicantReportsForAllClientsForBUClient?subId=" + formId+"&colName="+colName;		
