jQuery(document).ready(function() {
    ...
    jQuery('body').on('click', '#header-search-button-list', function() {
        jQuery(this).parents().find('.header-search-form-wrap-list:first').slideToggle(200);
    });
    var setupHeaderSearchformlist = function() {
        var headerSearchFormTopPositionList = jQuery('.header-top').outerHeight() + jQuery('.header-bottom').outerHeight();
        jQuery('.header-search-form-wrap-list').css({
            'top': headerSearchFormTopPositionList + 'px'
        });
    };
    setupHeaderSearchformlist();
    jQuery(window).on('load scroll resize', function() {
        setupHeaderSearchformlist();
    });
	
	/* Search Click search box appear */
